...
text_splitter = CharacterTextSplitter(separator="\n", chunk_size=0.00001, chunk_overlap=0)
...
def recommend_books(
    ...
    for _, row in recommendations.iterrows():
        ...
        aurthors_split = row["authors"].split(";")
        if len(aurthors_split) == 2:
            authors_str = f"{aurthors_split[0]} and {aurthors_split[1]}"
        elif len(aurthors_split) > 2:
            authors_str = f"{', '.join(aurthors_split[:-1])}, and {aurthors_split[-1]}"
        else:
